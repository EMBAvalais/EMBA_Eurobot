...
//	unsigned int8 ToSetBase=0;
	...
	int1 Module_inplace = 0;
  	...
  	driver_Pompes(0xA0, 0, 0, 0, 0, 0);		//D�sactive toute les pompes
	
	
	while(!Module_inplace) Module_inplace=__GoToZero();		//R�initialise la couronne
	go_driver_moteur(0xC2,0,DRV_MODE_VITESSE ,5000,DRV_ARRET_POSITION_RELATIVE,1);	//Pour que la couronne maintient la position
	delay_ms(100);
	Module_inplace = 0;
	while(!Module_inplace) Module_inplace=GoToZero_Cremaillere();		//R�initialise la cr�maill�re
	delay_ms(100);
